// The 30 juz (para) boundaries are a fixed, universal division of the Quran —
// identical across every Mushaf/edition, unrelated to any DB content, so this
// is safe as a static table rather than a DB round-trip.
...
export const JUZ_BOUNDARIES: readonly JuzBoundary[] = [
...
// Ordered list of surahs (fully or partially) contained in `juzNumber`, each
// with the ayah range that belongs to THIS juz — a surah split across a juz
// boundary shows only its portion, not the whole surah.
export function surahsInJuz(
  juzNumber: number,
  surahs: readonly { number: number; ayahCount: number }[],
): JuzSurahEntry[] {
  const boundary = JUZ_BOUNDARIES.find((b) => b.juz === juzNumber);
  if (!boundary) return [];

  const ayahCountBySurah = new Map(surahs.map((s) => [s.number, s.ayahCount]));
  const entries: JuzSurahEntry[] = [];
  for (let s = boundary.startSurah; s <= boundary.endSurah; s++) {
    const ayahStart = s === boundary.startSurah ? boundary.startAyah : 1;
    const ayahEnd = s === boundary.endSurah ? boundary.endAyah : (ayahCountBySurah.get(s) ?? ayahStart);
    entries.push({ number: s, ayahStart, ayahEnd });
  }
  return entries;
}
