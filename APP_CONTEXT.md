...
packages/api/src/
  ...
  db/
    ...
    models/category.model.ts    → CategoryModel
    ...
    migrations/
      ...
      ⚠️ Default `pnpm migrate` chain is steady-state ONLY: [0001, 0002, 0006, 0007, 0008, 0009, 0010]. Legacy one-time embedded-locale transforms (0003/0004/0005) are excluded from the default chain — reach via `--only <name>` (warns "LEGACY one-time transform").
  repositories/
...
- **Public web read path is cached + invalidated across deployments (refactor-hardening Phase 1).** Services no longer call `revalidateTag` directly — every mutation calls `invalidate([...tags])` (`packages/api/src/cache/invalidate.ts`), which revalidates locally AND POSTs the tags to web's `POST /api/revalidate` (secret-gated by `REVALIDATE_SECRET`). This is mandatory because mutations run in the **admin** deployment but the cached readers live in **web** — `revalidateTag` only invalidates its own deployment. Web pages/`api/v1` routes read through `apps/web/lib/cached-content.ts` (`getCachedPublishedPlaylists`/`getCachedCategories`/`getCachedPlaylistBySlug`/`getCachedTracksWithUrls`) — `unstable_cache` wrappers, 5-min TTL self-healing fallback, tagged to match what the services bust (**tracks use `playlistTag(id)`, NOT `PLAYLISTS_HOME`** — track mutations bust the per-playlist tag). Pages stay `force-dynamic` (nonce CSP); only the *data* is cached, not the HTML. New cached subpaths need a `@repo/api/cache/*` exports entry (added `./cache/tags` + `./cache/invalidate`). Date fields round-trip as ISO strings on a cache hit — every consumer already guards `Date | string` (`serializePlaylist`, `withIsoDates`, `toDisplayTrack`).
...
- **Migration chain is split into steady-state vs legacy** (fix/review-hardening, Phase 4): `scripts/migrate.ts` exports two arrays — `migrations` (default chain: `[0001, 0002, 0006, 0007, 0008, 0009, 0010]`, safe to run in full against any embedded-locale DB) and `legacyMigrations` (`[0003, 0004, 0005]`, one-time embedded-locale transforms from 2026-05). `pnpm migrate` / `pnpm migrate --dry-run` only touch the steady-state chain. `--only <name>` searches both lists and prints a "LEGACY one-time transform" warning if the name matches a legacy migration. `0003` additionally still self-guards via `dbHasEmbeddedLocaleDocs()`. To heal/backfill an already-embedded DB, run `pnpm migrate --only 0005-embedded-locale`: its merge loops no-op on `contentId`-less docs, `backfillSlugs()` fills missing slugs, and it drops+rebuilds indexes safely.
