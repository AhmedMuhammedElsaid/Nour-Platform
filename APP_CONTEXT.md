...
| Concern | Path | Note |
|---|---|---|
| **Extension** (Chrome MV3) | `apps/extension/` | New; Phase 1 complete (azan engine). See `apps/extension/APP_CONTEXT.md` for extension-specific details. |
| Extension source | `apps/extension/src/` | `background/` (scheduler) · `options/` (settings page) · `offscreen/` (audio player) · `lib/` (storage, cache, notify). |
| Extension build | `apps/extension/vite.config.ts` + `manifest.config.ts` | CRXJS generates MV3 manifest; Chrome/Firefox variants emit from one config. |
| Extension tests | `apps/extension/src/**/*.test.ts` | 12 unit tests (storage, fired-claim, scheduler). |
...
| Browser Extension (Chrome MV3) — Phase 1 ✅ | `feature/chrome-extension` (2026-06-25) | **Reliable closed-app azan on desktop.** New `apps/extension` MV3 extension with background scheduler + offscreen audio player. Reuses `@repo/shared-core` prayer-times logic (compute + predicates). Device-local only (no accounts). **Stack**: CRXJS v2 + Vite · `webextension-polyfill` (browser compat) · React + Tailwind tokens · `chrome.storage.local` (Zod-validated, key shapes mirror web/mobile `nour.*` localStorage). **Phase 1 (Azan engine)**: (1) **Storage** (`src/lib/storage.ts`): typed wrapper + Zod defaults. (2) **Fired-claim dedup** (`src/lib/fired-claim.ts`): 120ms settle window so two concurrent contexts fire adhan once per device (port of web's `fired-event-store`). (3) **Background scheduler** (`src/background/scheduler.ts`): `chrome.alarms` 1-min heartbeat → recompute from live clock → `recentlyMissedAdhan` (2-min catch-up window) → `claimFiredEvent` → notify + play + re-arm. No `isAdhanEventStale` needed (recompute-from-clock architecture is stale-proof). (4) **Offscreen audio** (`src/offscreen/`): one `<audio>` element, loaded on-demand, closed after playback. (5) **Audio routing** (`src/lib/audio-router.ts`): Chrome → offscreen, Firefox → managed player tab (Phase 2). (6) **Asset caching** (`src/lib/cache-manager.ts`): fetch `/audio/adhan.mp3` + `/adhan-fajr.mp3` into Cache API on install; offscreen resolves from cache first (offline playback), falls back to live URL. (7) **Options page** (`src/options/`): React form with location picker (city search + geolocation), method/madhab, per-prayer toggles, volume slider, azkar reminder offset. Storage writes trigger background re-arm via `storage.onChanged`. (8) **Notifications** (`src/lib/notify.ts`): Arabic strings hardcoded (no next-intl); clicking a notification opens the site or Adhkar reader URL. (9) **Manifest config** (`src/manifest.config.ts`): Chrome gets `service_worker` + `offscreen` permission; Firefox gets `background.scripts` + manifest additions (Phase 2). **Key differences from web**: (A) SW is ephemeral — no `setTimeout`, only `chrome.alarms`. (B) Notifications fire reliably with all tabs closed. (C) Storage persists across SW restart. (D) Offscreen document has full DOM/audio autoplay (no gesture needed). **Tests**: 12 unit tests (storage, fired-claim, scheduler, audio-router). **ADR**: `docs/adr/0008-browser-extension-crxjs.md`. **Deferred to Phase 2+**: new-tab dashboard, toolbar popup, audio-while-browsing (reuse offscreen player), Firefox port. |
