...
| Concern | Path | Note |
|---|---|---|
| **Extension** (Chrome + Firefox MV3) | `apps/extension/` | **ALL 12 roadmap phases complete** (P0–P11): azan engine, new-tab SPA (9 routes: home, playlist, search, adhkar, adhkar-read, quran, quran-read, bookmarks, prayer-times), audio player, Firefox port, full AR/EN i18n. Dual-target: `pnpm build:chrome` → `dist/chrome/`, `pnpm build:firefox` → `dist/firefox/`. See `apps/extension/APP_CONTEXT.md` + `website_to_chroom_plan.md` for extension-specific details. |
...
| Extension tests | `apps/extension/src/**/*.test.ts` | **42 unit tests / 7 files** (storage, fired-claim, scheduler, player-state, adhkar-progress, quran-progress, storage-prefs); vitest setup global polyfill mock. |
...
**Browser Extension: ALL 12 roadmap phases COMPLETE on `main` (head `84b81a9`, 2026-06-27).** New-tab is a full 9-route SPA mirroring every web feature (home, playlists, search, adhkar reader, quran reader, prayer-times, bookmarks). AR/EN bilingual. Light/dark theme. 42 tests / 7 files. Build: `pnpm --filter extension build:chrome` + `build:firefox`. Next step for the extension: load-unpacked testing → Chrome/Firefox store submission (see `website_to_chroom_plan.md` §deployment).
