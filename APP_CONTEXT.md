...
Audio MVP (Waves 0–5) + pre-deploy fixups + hardening sprint + **P2-A Categories** + **i18n A+B (AR/EN)** + **embedded-locale refactor** + **SoundCloud-UX & offline PWA & search** all landed and merged to `main` (HEAD = `0fd01bb`). Atlas migrations applied (search text indexes `0006` need running on prod — see deploy.md step 6).
