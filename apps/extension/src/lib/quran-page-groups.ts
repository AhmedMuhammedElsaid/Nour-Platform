import type { ReaderAyah } from "./content";
...
// ReaderAyah) — no new API call. Single pass, split whenever `page` changes; a
// group's `juz` is its first ayah's juz (mirrors the physical Madani mushaf,
// where a page's header juz is the juz its first line belongs to).
